...
check_swift_availability() {
    ...
    sudo su -c "hadoop distcp -D fs.swift.service.savanna.username=$OS_USERNAME -D fs.swift.service.savanna.tenant=$OS_TENANT_NAME -D fs.swift.service.savanna.password=$OS_PASSWORD /swift-test/test-file swift://$CONTAINER_NAME.savanna/" $HADOOP_USER
    check_return_code_after_command_execution -clean_hdfs `echo "$?"`

    sudo su -c "hadoop distcp -D fs.swift.service.savanna.username=$OS_USERNAME -D fs.swift.service.savanna.tenant=$OS_TENANT_NAME -D fs.swift.service.savanna.password=$OS_PASSWORD swift://$CONTAINER_NAME.savanna/test-file /swift-test/swift-test-file" $HADOOP_USER
    check_return_code_after_command_execution -clean_hdfs `echo "$?"`
