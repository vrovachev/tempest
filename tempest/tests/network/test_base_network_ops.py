...
class TestNetworkBasicOps(smoke.DefaultClientSmokeTest):
    ...
    @classmethod
    def tenant_by_name(cls, cfg):
        tenants =  filter(lambda x: x.name == cfg.tenant_name,
            cls.manager._get_identity_client(
            ).tenants.list())
    ...
    def setUpClass(cls):
        ...
        cfg = cls.config.network
        cls.tenant_id = TestNetworkBasicOps.tenant_by_name(cfg)
